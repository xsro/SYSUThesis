...
第四步，编译生成pdf文档。用比较专业的语言说，就是先用XeLaTeX编译一次，再用BibTeX编译一次，再用XeLaTeX编译两次。具体操作为：用TeXstudio打开main.tex文件，点击上方绿色双箭头（Build \& View）（图 \ref{fig:f5}），点击Tools->Bibliography（图 \ref{fig:f8}），再点击两次绿色单箭头（Compile）（图 \ref{fig:f6}），就能生成正确的pdf文档。这个过程用快捷键操作就是F5->F8->F6->F6。这么做的具体原理在此处并不赘述，之后会附上参考资料。
\input{figure/chap01/f5}
\input{figure/chap01/f8}
\input{figure/chap01/f6}
